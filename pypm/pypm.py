...
def main():  # pragma: nocover
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", default=False, help="Suppress diagnostic"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, help="Verbosity flag"
    )

    subparsers = parser.add_subparsers(
        title="pypm script", description="pypm subcommands", help="sub-command help"
    )
    ...
    parser_sim = subparsers.add_parser(
        "sim", help="Run simulations to generate observational data"
    )
    parser_sim.add_argument(
        ...
        action="store_true",
        default=False,
        help="Anonymize observation results",
    )
    ...
    parser_mip.add_argument(
        "-o",
        "--output",
        help="YAML file where results are stored",
        default="results.yaml",
    )
    parser_mip.add_argument(
        "-v",
        "--verbose",
        help="Output verbose results",
        action="store_true",
        default=False,
    )
    ...
    parser_unsup.add_argument(
        "-i", "--index", help="Index of problem to run", default=0
    )
    parser_unsup.add_argument(
        "-o",
        "--output",
    ...
    parser_vis.add_argument(
        "-o", "--output", help="HTML file where results are stored", default=None
    )
    parser_vis.add_argument(
        "-i", "--index", help="Index of alignment that is visualized", default=0
    )
    parser_vis.add_argument(
        "-t",
        "--type",
        help="Indicates the type of visualization generated: gannt, gannt-separation or labelling",
        default="gannt",
    )
    parser_vis.add_argument(
        "-r",
        "--rescale",
        help="Flag that indicates a rescaling of the y-axis",
        action="store_true",
        default=False,
    )
    ...
    parser_chunk = subparsers.add_parser(
        "chunk", help="Chunk the time steps in a process or observations"
    )
    parser_chunk.add_argument(
        "-s",
        "--step",
        help="Chunk step.  This is a string describing the chunking step: 2h, 4h, 3:55554h, 8h",
        default="3:55554",
    )
    parser_chunk.add_argument(
        "-c", "--csv", help="CSV file of observations", default=None
    )
    parser_chunk.add_argument(
        "-i",
        "--index",
        help="Name of the date-time column in the CSV file",
        default=None,
    )
    parser_chunk.add_argument("-p", "--process", help="YAML process file", default=None)
    parser_chunk.add_argument(
        "-o", "--output", help="Name of output file", default=None
    )
    ...
    if args.func == "sim":
        ...
        runsim(
            ...
            processfile=args.process_file,
            supervised=not args.unsupervised,
            outputfile=prefix + "_sim.yaml",
        )
        ...
        if args.type == "gannt":
            ...
            create_gannt_chart_with_separation_metric(
                args.process,
                args.results,
                output_fname=args.output,
                index=int(args.index),
