...
class BaseModel(object):
    ...
    def write_model(self, filename):
        ...
        self.M.write(filename, io_options=dict(symbolic_solver_labels=True))
    ...
    def perform_optimization(self):
        ...
        if self.config.solver_options:
            results = opt.solve(
                self.M, options=self.config.solver_options, tee=self.config.tee
            )
        else:
            results = opt.solve(self.M, tee=self.config.tee)
        if self.config.tee:  # pragma: no cover
