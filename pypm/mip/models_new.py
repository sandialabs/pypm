...
def add_objective_o(
    *,
    ...
    if supervised and len(count_data) > 0:
        ...
        def rsum_(m, k, t):
            return (
                sum(ar_count[j, k] * M.r[j, k, t] for j in J if (j, k) in JK) <= O[k][t]
            )
...
def add_xydef_constraints(*, M, T, J, p, q, E, gamma, max_delay, verbose=False):
    ...
    def stop_after_start_(m, j):
        return (
            sum(t * m.y[j, t] for t in T) - sum(t * m.x[j, t] for t in T) + 1
            == m.d[j] + m.g[j]
        )
...
def create_pyomo_model1(*, K, Tmax, J, E, p, O, S, count, sigma=None, verbose=False):
    ...
    add_simultenaity_constraints(
        M=M, J=J, sigma=sigma, T=T, Kall=Kall, count=count, J_k=J_k, verbose=verbose
    )
...
def create_pyomo_model2(*, K, Tmax, J, E, p, U, O, S, count, sigma=None, verbose=False):
    ...
    add_adefx_constraints(M=M, J=J, T=T, p=p, verbose=verbose)
    add_fixed_length_activities(M=M, T=T, J=J, p=p, verbose=verbose)
    add_rdef_constraints_unsupervised(
        M=M, K=Kall, JK=JK, T=T, O=O, U=U, verbose=verbose
    )
    add_simultenaity_constraints(
        M=M, J=J, sigma=sigma, T=T, Kall=Kall, count=count, J_k=J_k, verbose=verbose
    )
...
def create_pyomo_model3(
    *,
    ...
    add_objective(M=M, J=J, T=T, S=S, K=K, verbose=verbose)
    add_xydef_constraints(
        M=M, T=T, J=J, p=p, q=q, E=E, max_delay=max_delay, gamma=gamma, verbose=verbose
    )
...
def create_model3(
    *, observations, pm, timesteps, sigma=None, gamma=0, max_delay=0, verbose=False
):
    # Supervised
    # Fixed-length activities
    # No gaps within or between activities
    ...
    count = {name: pm.resources.count(name) for name in pm.resources}
    max_delay = {
        j: max_delay if pm[j]["max_delay"] is None else pm[j]["max_delay"] for j in pm
    }

    return create_pyomo_model3(
        K=K,
        Tmax=timesteps,
        J=J,
        E=E,
...
def create_pyomo_model5(
    *,
    ...
    add_adefzw_constraints(
        M=M, J=J, T=T, p=p, q=q, gamma=gamma, Tmax=Tmax, verbose=verbose
    )
    add_variable_length_activities_z(
        M=M, T=T, J=J, p=p, q=q, Tmax=Tmax, verbose=verbose
    )
    add_rdef_constraints_supervised(M=M, JK=JK, T=T, O=O, verbose=verbose)
    add_simultenaity_constraints(
        M=M, J=J, sigma=sigma, T=T, Kall=Kall, count=count, J_k=J_k, verbose=verbose
...
def create_pyomo_model78(
    *,
    ...
    add_objective_o(
        M=M, J=J, T=T, S=S, K=K, O=O, verbose=verbose, supervised=supervised
    )
    if not supervised:
        # add_rdef_constraints_supervised(M=M, JK=JK, T=T, O=O, verbose=verbose)
        # else:
    ...
    add_adefz_constraints(
        M=M, J=J, T=T, E=E, p=p, q=q, gamma=gamma, Tmax=Tmax, verbose=verbose
    )
    add_variable_length_activities_z7(
        M=M, T=T, J=J, p=p, q=q, Tmax=Tmax, verbose=verbose
    )
    add_simultenaity_constraints(
        M=M, J=J, sigma=sigma, T=T, Kall=Kall, count=count, J_k=J_k, verbose=verbose
...
def create_model10(
    *,
    ...
    add_rdef_constraints_unsupervised_more(
        M=M, K=Kall, JK=None, T=None, O=None, U=U, verbose=verbose
    )
    return M


def create_pyomo_model11_12(
    *,
    K,
    Tmax,
    J,
...
def create_model11_12(
    *,
    ...
    return create_pyomo_model11_12(
        K=K,
        Tmax=timesteps,
        J=J,
        E=E,
...
def create_model13_14(
    *,
    ...
    return create_pyomo_model13_14(
        K=K,
        Tmax=timesteps,
        J=J,
        E=E,
