...
def runsim(
    *,
    ...
    if configfile is None:
        assert (
            config is not None
        ), "Must specify the value of configfile or config options for runsim()"
    ...
    assert (
        timesteps > 0
    ), "Configuration file must specify 'timesteps' greater than zero"
    assert len(seeds) >= ntrials, "Configuration does not specify {} seeds".format(
        ntrials
    )
    ...
    if processfile is None:
        assert (
            process is not None
        ), "Must specify the value of processfile or process options for runsim()"
    ...
    for i in range(ntrials):
        ...
        sim = Simulator(
            ...
            observe_activities=observe_activities,
        )
        sim.run(seeds[i])
        ...
        if not supervised:
            observations = {
                "u{}".format(i): observations[key]
                for i, key in enumerate(sorted(observations.keys()))
            }
    ...
    contents = dict(
        _options=dict(
            comments=yamldata.get("comments", []),
            timesteps=timesteps,
            config=configfile,
            process=processfile,
            solver="glpk",
