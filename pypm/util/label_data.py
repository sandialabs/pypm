...
def label_data2(
    *,
    feature_label_csvfile,
    process_yamlfile,
    obs_csvfile,
    labeled_obs_csvfile="labeled_obs.csv",
    dirname=None
):
    #
    # Load data
    #
    assert os.path.exists(feature_label_csvfile), "Cannot find CSV file: {}".format(
        feature_label_csvfile
    )
    assert os.path.exists(process_yamlfile), "Cannot find YAML file: {}".format(
        process_yamlfile
    )
    ...
    df = pd.DataFrame(
        columns=["DateTime"] + list(sorted(name for name in process.resources))
    )
