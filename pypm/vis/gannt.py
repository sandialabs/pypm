...
def create_gannt_chart(
    ...
    assert results["solver"]["model"]["name"] in [
        "model3",
        "model4",
        "model5",
        "model6",
        ...
        "GSF-compact",
        "XSF-compact",
        "CompactMatches_FixedLengthActivities",
        "GSF-ED",
    ...
    if "datetime_schedule" in results["results"][index]:
        ...
        for activity in process["activities"]:
            ...
            if name not in alignment:
                print(
                    'Warning: Activity "{}" was not included in the process match'.format(
                        name
                    )
                )
                ...
                if "pre" in alignment[name]:
                    print(
                        'Warning: Activity "{}" omitted because it starts before the data time window.'.format(
                            name
                        )
                    )
                if "post" in alignment[name]:
                    print(
                        'Warning: Activity "{}" omitted because it ends after the data time window.'.format(
                            name
                        )
                    )
            else:
                ...
                data["Match Score"].append(
                    results["results"][index]["goals"]["match"].get(name, 0)
                )
                if "stop" in alignment[name]:
                    data["Stop"].append(alignment[name]["stop"])
                elif "last" in alignment[name]:
                    print(
                        "Warning: Using 'last' for activity {} because 'stop' is missing.".format(
                            name
                        )
                    )
...
def create_gannt_chart_with_separation_metric(
    process_fname, results_fname, output_fname=None, index=0, cmax=None, cmin=None
):
    assert os.path.exists(process_fname), "Unknown file {}".format(process_fname)
    assert os.path.exists(results_fname), "Unknown file {}".format(results_fname)
    import pandas as pd
    ...
    if "datetime_schedule" in results["results"][index]:
        ...
        for activity in process["activities"]:
            ...
            if name not in alignment:
                ...
                data["Activity"].append(name)
                data["Start"].append(alignment[name]["first"])
                data["Separation Score"].append(
                    results["results"][index]["goals"]["separation"].get(name, 0)
                )
                if "stop" in alignment[name]:
                    data["Stop"].append(alignment[name]["stop"])
                elif "last" in alignment[name]:
    ...
    if linear:
        fig = px.timeline(
            df, x_start="Start", x_end="Stop", y="Activity", color="Separation Score"
        )
        fig.layout.xaxis.type = "linear"
        df["delta"] = df["Stop"] - df["Start"]
        fig.data[0].x = df.delta.tolist()
