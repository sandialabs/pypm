...
def create_gannt_chart(process_fname, results_fname, output_fname=None, index=0):
    ...
    assert results['model'] in ['model3', 'model4', 'model5', 'model6', 'model7', 'model8','model10', 'model11', 'model12', 'model13', 'model14'], "Cannot visualize results in {}.  Expects results generated for model3-model8,model10-model14.".format(results_fname)
...
def create_gannt_chart_with_separation_metric(process_fname, results_fname, output_fname=None, index=0):
    assert os.path.exists(process_fname), "Unknown file {}".format(process_fname)
    assert os.path.exists(results_fname), "Unknown file {}".format(results_fname)
    import pandas as pd
    import plotly.express as px
    ...
    if 'datetime' in results:
        linear = False
        alignment = results['results'][index]['datetime_alignment']
        data = {'Activity':[], 'Start':[], 'Stop':[], 'Separation':[]}
        for activity in process['activities']:
            name = activity['name']
            if name not in alignment:
                print("Warning: Activity \"{}\" was not included in the process match".format(name))
                ...
                data['Activity'].append(name)
                data['Start'].append(alignment[name]['first'])
                data['Separation'].append(results['results'][index]['separation'].get(name,0))
                if 'stop' in alignment[name]:
                    data['Stop'].append(alignment[name]['stop'])
                elif 'last' in alignment[name]:
                    print("Warning: Using 'last' for activity {} because 'stop' is missing.".format(name))
    ...
    print("Generating Figure")
    if linear:
        fig = px.timeline(df, x_start="Start", x_end="Stop",  y="Activity", color="Separation")
        fig.layout.xaxis.type = 'linear'
        df['delta'] = df['Stop']-df['Start']
        fig.data[0].x = df.delta.tolist()
    else:
