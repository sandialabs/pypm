...
def create_labelling_matrix(process_fname, results_fname, output_fname=None, index=0):
    ...
    if len(df['Resources']) % 2 == 1:
        fill_colors = [['lightgrey'] + ['white', 'lightgrey'] * (len(df['Resources'])//2)]
    else:
        fill_colors = [['white', 'lightgrey'] * (len(df['Resources'])//2)]
    fig = go.Figure(data=[go.Table(header=dict(values=header, fill_color='grey', line_color='darkslategray', font=dict(color='white', size=12)),
                                   cells=dict(values=[df[h] for h in header],
