...
def _esimate_transition_and_start_probabilities(hidden_states, simulations):
    ...
    return hmm.get_start_probs(), hmm.get_transition_probs()
...
def estimate_hidden_state_parameters(*, simulations):
    ...
    #
    hidden_states = list(sorted(set({state for sim in simulations for state in sim})))
...
def create_hmm(
    *, observed, hidden_state_params, emission_params, no_zeros=False, no_zeros_tol=1e-6
    ...
    start_probs = copy.copy(hidden_state_params.start_probs)
    start_probs[()] = 1e-3
    start_probs = normalize_dictionary(start_probs)
