...
def run_tabu_labeling(config, constraints=[], nworkers=1, debug=False):
    ...
    if nworkers == 1:
        ...
        if config.labeling_restrictions:
            ls = PMLabelSearch_Restricted(config=config, constraints=constraints, labeling_restrictions=config.labeling_restrictions)
        else:
            ls = PMLabelSearch(config=config, constraints=constraints)
    else:
        ray.init(num_cpus=nworkers+1)
        if config.labeling_restrictions:
            ls = ParallelPMLabelSearch_Restricted(config=config, nworkers=nworkers, constraints=constraints, labeling_restrictions=config.labeling_restrictions)
        else:
    ...
    if config.labeling_restrictions:
        results['results'][0]['resource_feature_list'] = {k:[j for j in point_[k] if point_[k][j]] for k in point_}
    else:
