...
because this model solves more quickly than other matching models.
...
Here are the first lines of labeled data generated here:

.. literalinclude:: ../pypm/tests/t101/labeled.csv
    :lines: 1-30


Constraining Tabu Search
------------------------

Several tabu search representations are used in pypm to structure the
search process. The ``resource_feature_list`` supports resource-feature
constraints that restrict the space of mappings considered.  The following
configuration file specifies a YAML file that defines these constraints:
...
The ``resourceName`` key specifies the name of the resource.  The
``knownFeature`` key specifies a list of feature names that are always
associated with this resource.  The ``possibleFeature`` key specifies
a list of feature names that may be associated with this resources.
Tabu search fixes the ``knownFeature`` in its search, and it restricts
the search to consider features in the ``possibleFeature`` list.

The ``like`` key is used to provide a short-hand in complex applications
where multiple resources have similar characteristics.  The value
specified is a resource whose ``knownFeature`` and ``possibleFeature``
values are used for the resource in this list element.
